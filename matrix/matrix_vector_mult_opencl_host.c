...
int main(int argc, char** argv){
  int dim_matrix = 8192*2;
  int number_of_workitems = dim_matrix;
  int number_of_workitems_in_workgroup = 128;
  ...
  printf("calculate the product of A . V = R. Matrix dimension %d x %d, Vector Dimension %d with %d work items in %d workgroups\n",
    dim_matrix, dim_matrix, dim_matrix, number_of_workitems, workgroup_count);
  ...
  // get kernel source
  char* kernel_source = read_kernel_file("matrix_vector_mult_kernel.cl");
  ...
  size_t size_vector = sizeof(int) * dim_matrix;
  ...
  //
  kernel = clCreateKernel(program, "matrix_vector_mult", &err);
  ...
  v_mem = clCreateBuffer(context,  CL_MEM_COPY_HOST_PTR,  size_vector, v, NULL);
  c_mem = clCreateBuffer(context,  CL_MEM_WRITE_ONLY, size_vector, NULL, NULL);

  if (!a_mem || !v_mem || !c_mem)
  {
  ...
  err |= clSetKernelArg(kernel, 1, sizeof(cl_mem), &v_mem);
